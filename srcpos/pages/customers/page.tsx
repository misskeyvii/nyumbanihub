...
import { customers as seedCustomers, type Customer } from '@/mocks/customers';
...
export default function Customers() {
  ...
  const purchaseHistory = viewing ? sales.filter((s) => s.customer === viewing.name) : [];
  ...
  return (
    <div className="mx-auto max-w-6xl space-y-6">
      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-3">
        {[
          { label: 'Total Customers', value: String(items.length), icon: 'ri-user-line', tone: 'bg-primary-100 text-primary-700' },
          { label: 'Repeat Customers', value: String(items.filter((c) => c.visits > 1).length), icon: 'ri-repeat-line', tone: 'bg-secondary-100 text-secondary-700' },
          { label: 'Lifetime Value', value: formatMoney(items.reduce((s, c) => s + c.totalSpent, 0)), icon: 'ri-coins-line', tone: 'bg-accent-100 text-accent-700' },
        ].map((s) => (
            ...
            <tbody>
              {filtered.map((c) => (
                <tr key={c.id} className="border-b border-background-100 last:border-0 hover:bg-background-50">
                  ...
                  <td className="px-5 py-3 font-semibold text-foreground-900">{formatMoney(c.totalSpent)}</td>
                  <td className="px-5 py-3 text-foreground-600">{c.lastVisit ? formatDate(c.lastVisit) : '—'}</td>
                  <td className="px-5 py-3">
        ...
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-2 gap-3">
            <div className="rounded-md bg-background-100 p-3">
              ...
              <p className="font-heading text-lg font-bold text-foreground-950">{formatMoney(viewing?.totalSpent || 0)}</p>
            </div>
            <div className="rounded-md bg-background-100 p-3">
              ...
              <div className="space-y-2">
                {purchaseHistory.map((s) => (
                  <div key={s.id} className="flex items-center justify-between rounded-md bg-background-100 p-3">
                    <div>
                      <p className="text-sm font-medium text-foreground-900">{s.receiptNo}</p>
