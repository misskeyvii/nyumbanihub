        ...
        <div className="max-w-7xl mx-auto px-4 md:px-6 py-10">
          ...
          {loading ? (
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 md:gap-5">
              {providers.map((p) => {
                const providerName = p.business_name || p.name;
                const providerAvatar = p.business_avatar_url || p.avatar_url;
                    ...
                    <Link to={`/provider/${p.id}`} className="block">
                      <div className="relative">
                        <div className="w-full h-48 overflow-hidden bg-gray-100 flex items-center justify-center">
                          {providerAvatar ? (
                            <img src={providerAvatar} alt={providerName} className="w-full h-full object-cover object-top group-hover:scale-105 transition-transform duration-500" />
                          ) : (
                            <span className="text-5xl font-bold text-gray-300">{providerName?.[0]?.toUpperCase()}</span>
                          )}
                        </div>
                      ...
                      <div className="px-4 pt-3 pb-1">
                        <h3 className="font-bold text-gray-900 text-sm leading-snug mb-1">{providerName}</h3>
                        {p.county && (
